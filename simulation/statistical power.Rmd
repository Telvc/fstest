...
# plot of statistical power with respect to sample size
...
sample_cl1 = mvrnorm(m/2, rep(-div,n_curve*K), (deca*cov_lpe) %x%diag(n_curve)) + sig*mvrnorm(m/2, rep(0,n_curve*K), diag(nrow(cov_rq%x%diag(n_curve))))
sample_cl2 = mvrnorm(m/2, rep(div,n_curve*K), (deca*cov_lpe) %x%diag(n_curve)) + sig*mvrnorm(m/2, rep(0,n_curve*K), diag(nrow(cov_rq%x%diag(n_curve))))
...
```
```{r}
pow1 = array(pow1,c(15,8))
pow1

sample_sizes <- seq(30, 100, length.out = ncol(pow1))
...
ggsave(
  filename = "C:/Users/11132/Downloads/Research/Duke/anru/selective inference/figure&data/statistical power/dis.pdf", 
  width = 9,           
  height = 6,         
  units = "in",        
  dpi = 1000
...
# plot of statistical power with respect to cluster difference in means
...
sampletimes = 30 #number of independent samples
m = 60
n_curve = 1
M = 500
sigm = c(0.1) #seq(0,3,0.3)
...
print(countp)
print(type1err/countp)
if(countp != 0) power3 = c(power3, type1err/countp)
else power3 = c(power3, NA)
  }
}
...
seq(0.3,1.2, length.out = ncol(pow2))
```
...
length(seq(0.3,1.5,0.1))
array(power3,c(sampletimes,length(seq(0.3,1.3,0.1))))

pow2 = array(power3,c(sampletimes,length(seq(0.3,1.5,0.1))))
pow2[,7] = array(power3,c(15,1))

pow1 = pow2
pow2 = pow2[,-c(9,10)]
pow2 = pow2[,-c(9)]

sample_sizes <- seq(0.3,1, length.out = ncol(pow2))
